...
lmdir=`pwd`/data/WSJ/local/nist_lm
mkdir -p $dir $lmdir
local=`pwd`/local
utils=`pwd`/utils
...
cat $CORPUS/wsj1/doc/indices/si_tr_s.ndx \
  ...
  | grep -v wsj0/si_tr_s/401 > train.flist
...
cat $CORPUS/wsj0/doc/indices/test/nvp/si_et_20.ndx | \
  $local/cstr_ndx2flist.pl $CORPUS | sort > eval.flist

# Dev-set for Nov'93 (503 utts)
cat $CORPUS/wsj1/doc/indices/h1_p0.ndx | \
  $local/cstr_ndx2flist.pl $CORPUS | sort > dev.flist
