...
for l in `seq 0 $[$num_lang-1]`; do
  ...
  multi_tree_dirs[$l]=${args[$l+$num_lang]}
  multi_ali_dirs[$l]=${args[$l+2*$num_lang]}
  multi_egs_dirs[$l]=${args[$l+3*$num_lang]}
done
...
for lang_index in `seq 0 $[$num_lang-1]`; do
    ...
    chain_dir=${multi_tree_dirs[$lang_index]}
  ...
  if [ ! -d "$egs_dir" ]; then
    echo "$0: Generate egs for language $lang_index"
    ...
    steps/nnet3/chain/get_egs.sh $egs_opts "${extra_opts[@]}" \
        --frames-per-iter $samples_per_iter --stage $stage --generate-egs-scp $generate_egs_scp \
        --frames-per-eg $frames_per_eg --frame-subsampling-factor $frame_subsampling_factor --cmd "$cmd" $egs_opts \
        $data $chain_dir $ali_dir $egs_dir || exit 1;
  fi
done
