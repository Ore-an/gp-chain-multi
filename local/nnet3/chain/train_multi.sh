...
cmd=run.pl
num_epochs=4      # Number of epochs of training (for first language);
...
parallel_opts=""  # default suitable for CPU-based training.
  # by default we use 16 threads; this lets the queue know.
  # note: parallel_opts doesn't automatically get adjusted if you adjust num-threads.
combine_num_threads=8
combine_parallel_opts=""  # queue options for the "combine" stage.
...
if [ $# -lt 6 -o $[$#%2] -ne 0 ]; then
  ...
  echo "Usage: $0 [opts] <ali0> <tree0> <egs0> <ali1> <tree1> <egs1> ... <aliN-1> <treeN-1> <egsN-1> <input-model> <exp-dir>"
  echo " e.g.: $0 data/train exp/tri6_lat exp/tri6_egs exp_lang2/tri6_ali exp_lang2/tri6_egs exp/dnn6a/10.mdl exp/tri6_multilang"
  echo ""
...
for lang in $(seq 0 $(($num_lang-1))); do
  alidir[$lang]=${argv[$lang*3+1]}
  tree_dir[$lang]=${argv[$lang*3+1]}
  egs_dir[$lang]=${argv[$lang*3+2]}
  for f in ${egs_dir[$lang]}/info/frames_per_eg ${egs_dir[lang]}/cegs.1.ark ${tree_dir[$lang]}/ali.1.gz ${tree_dir[$lang]}/tree; do
...
input_model_pdfs=$(nnet3-am-info $input_model | grep "output-node" | grep -Po "dim=\d*" | cut -d '=' -f 2)
input_model_pdfs=$(echo $input_model_pdfs | cut -d ' ' -f 1)
...
nnet3-am-copy --learning-rate=$initial_learning_rate $input_model $dir/0/0.mdl

if nnet3-am-info --print-args=false $dir/0/0.mdl | grep SumGroupComponent 2>/dev/null; then
...
# pretty sure nnet3 doesn't have sum group
...
if [ $stage -le -4 ]; then
  ...
  for lang in $(seq 1 $[$num_lang-1]); do
    ...
    num_targets=$(tree-info ${tree_dir[$lang]}/tree | grep num-pdfs | awk '{print $2}')
    learning_rate_factor=$(echo "print 0.5/$xent_regularize" | python)

    mkdir -p $dir/$lang/configs
    cat <<EOF > $dir/$lang/configs/network.xconfig
...
EOF
    steps/nnet3/xconfig_to_configs.py --existing-model $input_model \
				      --xconfig-file $dir/$lang/configs/network.xconfig \
				      --config-dir $dir/$lang/configs

    $train_cmd $dir/log/generate_input_mdl.log \
	       nnet3-init --srand=1 $input_model $dir/$lang/configs/final.config $dir/$lang/0.raw || exit 1;
    $train_cmd $dir/log/init_input_mdl.log \
	       nnet3-am-init $dir/$lang/0.trans_mdl $dir/$lang/0.raw $dir/$lang/0.mdl || exit 1;
...
# Should be useless for LF-MMI
# if [ $stage -le -3 ]; then
#   echo "Training transition probabilities and setting priors"
#   for lang in $(seq 0 $[$num_lang-1]); do
#     $cmd $dir/$lang/log/train_trans.log \
#       nnet3-am-train-transitions $dir/$lang/0.mdl "ark:gunzip -c ${alidir[$lang]}/ali.*.gz|" $dir/$lang/0.mdl \
#       || exit 1;
#   done
# fi
...
num_archives_expanded0=$[$num_archives0*$frame_subsampling_factor]
...
num_archives_to_process=$[($num_epochs*$num_archives_expanded0)]

num_iters=$[($num_archives_to_process)/${num_jobs_nnet_array[0]}]
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    for lang in $(seq 0 $[$num_lang-1]); do
      ...
      $cmd $dir/$lang/log/compute_prob_valid.$x.log \
        nnet3-chain-compute-prob --xent-regularize=$xent_regularize \
			     --leaky-hmm-coefficient=0.1 \
			     --l2-regularize=0.00005 \
			     "nnet3-am-copy --raw=true $dir/$lang/$x.mdl - |" $dir/$lang/den.fst \
			     "ark,bg:nnet3-chain-copy-egs ark:${egs_dir[$lang]}/valid_diagnostic.cegs ark:- | nnet3-chain-merge-egs --minibatch-size=1:64 ark:- ark:- |" &
      ...
      $cmd $dir/$lang/log/compute_prob_train.$x.log \
        nnet3-chain-compute-prob --xent-regularize=$xent_regularize \
			     ...
			     --l2-regularize=0.00005 \
			     "nnet3-am-copy --raw=true $dir/$lang/$x.mdl - |" $dir/$lang/den.fst \
			     "ark,bg:nnet3-chain-copy-egs ark:${egs_dir[$lang]}/train_diagnostic.cegs ark:- | nnet3-chain-merge-egs --minibatch-size=1:64 ark:- ark:- |" &
      
      if [ $x -gt 0 ] && [ ! -f $dir/$lang/log/mix_up.$[$x-1].log ]; then
        $cmd $dir/$lang/log/progress.$x.log \
	     nnet3-am-info $dir/$lang/$x.mdl '&&' \
	     nnet3-show-progress --use-gpu=no \
	     "nnet3-am-copy --raw=true $dir/$lang/$[$x-1].mdl - |" \
	     "nnet3-am-copy --raw=true $dir/$lang/$x.mdl - |" &
      fi
    done
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        ...
        for n in $(seq $this_num_jobs_nnet); do
          ...
          frame=$[(($archive_index+($k/$this_num_archives))%$frame_subsampling_factor)];

          $cmd --gpu 1 $dir/$lang/log/train.$x.$n.log \
             nnet3-chain-train --read-cache=$dir/$lang/cache.$x \
	       --write-cache=$dir/$lang/cache.$[$x+1] \
	       --xent-regularize=$xent_regularize \
	       --leaky-hmm-coefficient=0.1 \
	       --l2-regularize=0.00005 \
	       --apply-deriv-weights=false \
	       --max-param-change=$max_param_change \
	       --srand=$x $dir/$lang/$x.mdl $dir/$lang/den.fst \
	       "ark,bg:nnet3-chain-copy-egs --frame-shift=$frame --keep-proportion=$this_keep_proportion ark:${egs_dir[$lang]}/cegs.$archive.ark ark:- | nnet3-chain-shuffle-egs --buffer-size=$shuffle_buffer_size --srand=$x ark:- ark:- | nnet3-chain-merge-egs --minibatch-size=$this_minibatch_size ark:- ark:- |" \
            $dir/$lang/$[$x+1].$n.raw || touch $dir/.error &
        done
      done
      wait
    )
    ...
    (
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        this_num_jobs_nnet=${num_jobs_nnet_array[$lang]}
        nnets_list=$(for n in `seq 1 $this_num_jobs_nnet`; do echo $dir/$lang/$[$x+1].$n.raw; done)
        # average the output of the different jobs.
        $cmd $dir/$lang/log/average.$x.log \
	     nnet3-average $nnets_list - \| \
	     nnet3-copy --learning-rate=$learning_rate - $dir/$lang/$[$x+1].tmp.mdl || touch $dir/.error &

	 # nnet3-chain-combine --num-iters=20 \
	 #     --l2-regularize=0.00005 --leaky-hmm-coefficient=0.1 \
	 #     --enforce-positive-weights=true \
	 #     --verbose=3 $dir/$lang/den.fst $nnets_list \
	 #     "ark,bg:nnet3-chain-copy-egs ark:${egs_dir[$lang]}/combine.cegs ark:- | \
         #            nnet3-chain-merge-egs ark:- ark:- |" - \| \
	 #     nnet3-am-copy 

	#- $dir/$lang/$[$x+1].tmp.mdl || touch $dir/.error & 
      done
      wait
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        this_num_jobs_nnet=${num_jobs_nnet_array[$lang]}
        for n in `seq 1 $this_num_jobs_nnet`; do rm $dir/$lang/$[$x+1].$n.raw; done
      done
    )
    ...
    $cmd $dir/log/average.$x.log \
	 nnet3-average-ll --weights=$weights_csl --skip-layers=-1:-2:-6 \
	 $nnets_list - \| \
	 nnet3-am-copy --set-raw-nnet=- $dir/0/$x.mdl \
	 $dir/0/$[$x+1].mdl  || exit 1;
    ...
    for lang in $(seq 1 $[$num_lang-1]); do
      ...
      $cmd $dir/$lang/log/combine_average.$x.log \
	   nnet3-average-ll --weights=0.0:1.0 --skip-layers=-1:-2:-6 \
           $dir/$lang/$[$x+1].tmp.mdl $dir/0/$[$x+1].mdl - \| \
	   nnet3-am-copy --set-raw-nnet=- \
	   $dir/$lang/$x.mdl \
	   $dir/$lang/$[$x+1].mdl || exit 1;
    done
    # nnet3-am-copy --set-raw-nnet=$dir/$lang/$[$x+1].tmp.mdl \
    # 	   $dir/$lang/$x.mdl - \| \
    ...
    for lang in $(seq 0 $[$num_lang-1]); do # mix up.
      ...
      rm $dir/$lang/cache.$x 2>dev/null
    done
  fi
  x=$[$x+1]
done
...
if [ $stage -le $num_iters ]; then
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    if [ $max_models_combine -lt $num_models_combine ]; then
      ...
      for n in $(seq $max_models_combine); do
        ...
        nnets_list[$[$n-1]]="nnet3-average $sub_list - |"
    ...
    num_egs=`nnet3-chain-copy-egs ark:${egs_dir[$lang]}/combine.cegs ark:/dev/null 2>&1 | tail -n 1 | awk '{print $NF}'`
 ...
 # --initial-model=100000 --num-lbfgs-iters=40 --use-gpu=no \
 #        --num-threads=$combine_num_threads \
 #        --verbose=3 --minibatch-size=$mb
    $cmd $combine_parallel_opts $dir/$lang/log/combine.log \
	 nnet3-chain-combine --xent-regularize=$xent_regularize \
			     --leaky-hmm-coefficient=0.1 \
			     --l2-regularize=0.00005 \
			     --enforce-positive-weights=true \
			     $dir/$lang/den.fst "${nnets_list[@]}" \
			     "ark,bg:nnet3-chain-copy-egs ark:${egs_dir[$lang]}/combine.cegs ark:- | \
                              nnet3-chain-merge-egs --minibatch_size=$minibatch_size ark:- ark:- |" - \| \
			     nnet3-am-copy --set-raw-nnet=- $dir/$lang/$x.mdl $dir/$lang/final.mdl || touch $dir/.error &
  done
  wait
...
if [ $stage -le $[$num_iters+1] ]; then
  for lang in $(seq 0 $[$num_lang-1]); do  
    ...
    $cmd $dir/$lang/log/compute_prob_valid.final.log \
      nnet3-chain-compute-prob $dir/$lang/final.mdl $dir/$lang/den.fst ark:${egs_dir[$lang]}/valid_diagnostic.cegs &
    $cmd $dir/$lang/log/compute_prob_train.final.log \
      nnet3-chain-compute-prob $dir/$lang/final.mdl $dir/$lang/den.fst ark:${egs_dir[$lang]}/train_diagnostic.cegs &
  done
  wait
fi
...
if [ $stage -le $[$num_iters+2] ]; then
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    #$cmd JOB=1:$num_jobs_compute_prior $dir/$lang/log/get_post.JOB.log \
     # nnet3-chain-copy-egs --frame=random --srand=JOB ark:${egs_dir[$lang]}/egs.1.ark ark:- \| \
      #nnet3-chain-subset-egs --srand=JOB --n=$prior_subset_size ark:- ark:- \| \
     # nnet3-compute-from-egs "nnet-to-raw-nnet $dir/$lang/final.mdl -|" ark:- ark:- \| \
  ...
  for lang in $(seq 0 $[$num_lang-1]); do
    ...
    echo "(Commented out, should be unnecessary for LF-MMI) Re-adjusting priors based on computed posteriors for language $lang"
   # $cmd $dir/$lang/log/adjust_priors.final.log \
    #  nnet3-adjust-priors $dir/$lang/final.mdl $dir/$lang/post.vec $dir/$lang/final.mdl || exit 1;
  done
fi
