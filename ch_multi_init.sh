...
decode_lang_list=(FR CZ GE PO SP)             # (FR SP GE PO CZ)
...
for lang_index in `seq 0 $[$num_langs-1]`; do
  ...
  multi_egs_dirs[$lang_index]=exp/xe2ch/chain/multi-${lang_list[$lang_index]}-egs
...
if [ $stage -le 6 ]; then
  ...
  mkdir -p $dir/configs
  [ -f $dir/configs/network.xconfig ] && rm $dir/configs/network.xconfig
  ...
  for l in `seq 0 $[$num_langs-1]`; do
    ...
    echo " relu-batchnorm-layer name=prefinal-chain-$lan input=tdnn6.batchnorm dim=$dim target-rms=0.5"
    echo " output-layer name=output-$lan include-log-softmax=false dim=$num_targets max-change=1.5"
    
    # out for xent regularizing branch
    echo " relu-batchnorm-layer name=prefinal-xent-$lan input=tdnn6.batchnorm dim=$dim target-rms=0.5"
    echo " output-layer name=output-$lan-xent dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5"
  done >> $dir/configs/network.xconfig

  cp  $dir/configs/network.xconfig  $dir/configs/network.xconfig.bk  
  lzero=${lang_list[0]}
  sed -e "s/output-$lzero/output/g" -e "s/output-$lzero-xent/output-xent/g" $dir/configs/network.xconfig.bk >  $dir/configs/network.xconfig
  inmdl=$dir/configs/in.raw
  nnet3-copy --edits="remove-output-nodes name=output-*;remove-orphans;rename-node old-name=output new-name=output-old" $srcmdl $inmdl
  local/xconf.py --existing-model $inmdl\
		 --xconfig-file $dir/configs/network.xconfig \
		 --config-dir $dir/configs \

     $train_cmd $dir/log/generate_input_mdl.log \
	   nnet3-copy --edits="set-learning-rate-factor name=* learning-rate-factor=$adapt_lr" $inmdl - \| \
	   nnet3-init - $dir/configs/final.config $dir/configs/temp.raw || exit 1;

    nnet3-copy --edits="remove-output-nodes name=output-old;remove-orphans;rename-node old-name=output new-name=output-$lzero;rename-node old-name=output-xent new-name=output-$lzero-xent;" \
	       $dir/configs/temp.raw  \
	       $dir/0.raw

  for l in `seq 0 $[$num_langs-1]`;do
  lang=${lang_list[$l]}
  nnet3-copy --edits="rename-node old-name=output-$lang new-name=output-$l;rename-node old-name=output-$lang-xent new-name=output-$l-xent" $dir/0.raw $dir/0.raw
  done
...
if [ $stage -le 13 ]; then
  ...
  for lang_index in `seq 0 $[$num_decode_lang-1]`; do
    graph_dir=exp/chain/multi_all/${decode_lang_list[$lang_index]}/graph_tgpr_sri
