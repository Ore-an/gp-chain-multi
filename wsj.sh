...
if [ $stage -le 0 ]; then
  ...
  #utils/prepare_lang.sh data/WSJ/local/dict_nosp \
   #                    "<SPOKEN_NOISE>" data/WSJ/local/lang_tmp_nosp data/WSJ/lang_nosp || exit 1;
  ...
  for x in train eval dev; do
    steps/make_mfcc.sh --cmd "$train_cmd" --nj 10 data/WSJ/$x || exit 1;
    steps/compute_cmvn_stats.sh data/WSJ/$x || exit 1;
  done
...
if [ $stage -le 5 ]; then
  ...
  steps/get_prons.sh --cmd "$train_cmd" \
    data/WSJ/train data/WSJ/lang_nosp exp/WSJ/tri3b || exit 1;
  utils/dict_dir_add_pronprobs.sh --max-normalize true \
    data/WSJ/local/dict_nosp \
    exp/WSJ/tri3b/pron_counts_nowb.txt exp/WSJ/tri3b/sil_counts_nowb.txt \
    exp/WSJ/tri3b/pron_bigram_counts_nowb.txt data/WSJ/local/dict || exit 1

  utils/prepare_lang.sh data/WSJ/local/dict \
    "<SPOKEN_NOISE>" data/WSJ/local/lang_tmp data/WSJ/lang || exit 1;

  for lm_suffix in tgpr; do
    mkdir -p data/WSJ/lang_test_${lm_suffix}
    cp -r data/WSJ/lang/* data/WSJ/lang_test_${lm_suffix}/ || exit 1;
    rm -rf data/WSJ/lang_test_${lm_suffix}/tmp
    cp data/WSJ/lang_nosp_test_${lm_suffix}/G.* data/WSJ/lang_test_${lm_suffix}/
  done
